# fetchers.py — Audio source fetchers: Saavn, YT Music, SoundCloud, Piped, Invidious, yt-dlp
            ...
            for url in _KEEPALIVE_URLS:
                try:
                    ...
                    if r.status_code == 200:
                        _cb.record_success(url.split('/')[2])  # GODMODE-09: CB track
                ...
                try:
                    ...
                    for song in raw:
                        ...
                        _ok, _conf, _reason = _is_confirmed_match(
                            ...
                            min_conf=0.75,  # FIX-D4: raised from 0.70
                        )
                        if not _ok:
...
def fetch_from_ytmusic(title: str, artist: str = '') -> Optional[dict]:
    # GODMODE-05: circuit breaker check
    if not _cb.is_allowed('ytmusic'):
        log.debug('[CB] ytmusic OPEN — skipping')
        return None
...
def fetch_from_ytdlp(title, artist='') -> Optional[dict]:
    ...
    if not _cb.is_allowed('youtube'):
        log.debug('[CB] youtube/ytdlp OPEN — skipping')
        return None
    ...
    ydl_opts = {
        ...
        'extract_flat':   False,
        'noplaylist':     True,
        # FIX-21: Rotate User-Agent for yt-dlp
        'http_headers':   {'User-Agent': random.choice(_YTDLP_USER_AGENTS)},
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # BUG-4 FIX: was 0.50 — too low, caused wrong songs to cache
            if best_conf >= 0.65:
                _l1_audio.set(l1_key, result)
...
def fetch_from_soundcloud(title, artist='') -> Optional[dict]:
    ...
    if not _cb.is_allowed('soundcloud'):
        log.debug('[CB] soundcloud OPEN — skipping')
        return None
        ...
        for endpoint in endpoints:
            try:
                ...
                if best_url:
                    ...
                    # FIX-11 equivalent: raise ID verify threshold to 0.72
                    if expected_title and id_result.get('title'):
                        ...
                        if _id_verify_conf < 0.72:
                            ...
                            return None
                    # Also reject if fetched song is a version user didn't want
...
def fetch_saavn_parallel(query, title: str = '', artist: str = '', language: str = ''):
    ...
    log.info(f"[Parallel] ✓ '{best['title']}' conf={best_conf:.2f} q={best['quality']} mirrors={len(all_results)}")
    return best
...
# FIX-13: Added language parameter to signature
def fetch_from_jiosavan(title: str, artist: str = '', language: str = '') -> Optional[dict]:
    ...
    try:
        t0 = time.time()
        # FIX-13: Pass language to JioSavan API
...
def play_song():
    ...
    def _check_cache_entry(entry):
        if not entry or not entry.get('url'):
            return None
        _ct = entry.get('title', '')
        if _is_remix_or_cover(_ct) or _is_slowed_reverb(_ct) or _is_live_version(_ct):
        ...
        if title and _cached_title:
            ...
            # FIX-11: Raised threshold 0.65 → 0.72
    ...
    # FIX-18: song_index fast path — skips all search, direct ID fetch
    if not audio_url and title:
        _indexed_id = _song_index_get(title, artist)
        if _indexed_id and _indexed_id != song_id:
            _idx_result = _fetch_saavn_by_id(
                _indexed_id, expected_title=title, expected_artist=artist)
            if _idx_result and _idx_result.get('url'):
                ...
                if title and _play_ck_title:
                    _early_cache = {**_idx_result, 'source': 'saavn', 'confidence': 0.95,
    ...
    if not audio_url and title:
        ...
        # GODMODE-06: use adaptive confidence floor for this query
        _MIN_FALLBACK_CONF = _conf_tuner.get_floor(title, artist)

        # Phase 1: JioSavan fast path
        _phase1_futures = {
        ...
        if not audio_url:
            ...
            if _fb_candidates:
                # FIX-12: Pure confidence ordering — no arrival-time bias
                _fb_candidates.sort(key=lambda x: -x[1])

                # Source priority tiebreak within 0.03 confidence
                _SOURCE_PRI = {
                ...
                log.info(
                    ...
                    f"candidates={len(_fb_candidates)} title='{_winner_res.get('title')}' q={quality}"
                )

    # Step 6: Broad YouTube last-resort
    if not audio_url and title:
    ...
    if not audio_url:
        ...
        # GODMODE-06: record miss so tuner can lower floor next time
    ...
    # GODMODE-06: record successful accept + source perf
    ...
    # FIX-19: Self-learn — write successful plays to song_index
    if confidence >= 0.85 and title and song_id and source in ('saavn', 'jiosavan'):
        _executor_cache.submit(
            _song_index_put, title, artist, song_id, title, _best_art
        )
...
# ═══════════════════════════════════════════════════════════════════════════════
# FIX-20: /api/prefetch — Warm next songs into cache
# ═══════════════════════════════════════════════════════════════════════════════
...
def prefetch_songs():
    ...
    def _do_prefetch(s):
        ...
        if _l1_saavn.get(_ck): return  # already warm
        if _id:
            ...
            if result and result.get('url'):
                ...
                log.info(f"[Prefetch] ✓ Warmed: '{_title}'")
                return
        if _title:
            ...
            for _qv in build_query_variants(_title, _artist, '')[:2]:
                ...
                if _res and _res.get('url'):
                    ...
                    log.info(f"[Prefetch] ✓ Warmed via search: '{_title}'")
                    return

    for song in queue:
        # BUG-5 FIX: use background pool so audio pool stays free for playback
